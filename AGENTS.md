...
**どうでもいいこと**は、あとから道具で一括に変えられる決定です。コードの整形、`use` の並び、Blade の書き方、Taskfile のタスク名、`config/` のキー名、Docker イメージの選び方。Laravel と PHP コミュニティがいまやっている形にそのまま合わせ、議論も ADR も書きません。ECS と Rector が既定の規則集で直せる範囲はすべてここです。合わせる目的は、次に来る人が読む Laravel の資料と一致させることにあり、流行が変われば Rector のセットを上げて追随します。

**重大なこと**は、自分たちが管理しない相手(他のシステム、別の言語で書かれたクライアント、3 年後の自分たち)と共有する決定です。仕様のある標準に従い、フレームワークの便利さより優先します。HTTP のステータスと `Location`、RFC 3339 の日時、JSON と JSON Lines、終了コードと標準入出力の使い分け、PSR-4、PSR-20(`Psr\Clock\ClockInterface`)、SQL の DDL、UTF-8。`Domain/` が Carbon を持ち込まず `DateTimeImmutable` で日時を表し、`Console/` が `--json` で 1 行 1 レコードを出し、API が 201 と `Location` を返すのはこの判断です。標準を破りたくなったら、設計が間違っている合図と考えます。
