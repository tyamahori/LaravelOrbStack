...
設計の柱は四つです。**機能ごとにパッケージを切り(package by feature)、パッケージの中では依存を内側に向け(clean architecture)、部品は一つの仕事だけをして組み合わせ(Unix 哲学)、フレームワークは外側の層でだけ、しかし遠慮なく使う**。以下はその四つを、このリポジトリのディレクトリと検査ツールに落としたものです。
