...
このファイルはエージェント(Claude Code、Codex、OMP)向けの正本で、`CLAUDE.md` はここへのシンボリックリンクです。環境構築・コマンド一覧・ヘルパ禁止の仕組みは `README.md` に書いてあり、ここでは重複させません。機械横断の作法(コミット、日本語、ツール選択)はグローバル指示に任せ、このリポジトリ固有の「どこに何を置き、何に依存してよいか」だけを決めます。前提にする skill(`software-design`、`test-design`、`natural-japanese`、`japanese-tech-writing`、`github-pr-review`、`github-pr-respond`、`diagnosing-bugs`)は `.agents/skills/` に実体を置き、`.claude/skills/` から相対リンクしています。正本は tyamahori/dotfiles の `agents/skills/` で、更新はそこからコピーし直します。
...
決定の根拠は三つに分けます。どうでもいいことは流行に従い、重大なことは標準に従い、ドメインのことは自分で設計する(小津安二郎の「なんでもないことは流行に従い、重大なことは道徳に従う。芸術のことは自分に従う」を @koriym 氏がソフトウェアに置き換えたもの)。どれに入るかは、その決定が間違っていたときに誰がいつ払うかで決まります。

**どうでもいいこと**は、あとから道具で一括に変えられる決定です。コードの整形、`use` の並び、Blade の書き方、Taskfile のタスク名、`config/` のキー名、Docker イメージの選び方。Laravel と PHP コミュニティがいまやっている形にそのまま合わせ、議論も ADR も書きません。ECS と Rector が既定の規則集で直せる範囲はすべてここです。合わせる理由は正しさではなく、次に来る人が読む Laravel の資料と一致していることで、流行が変われば Rector のセットを上げて追随します。

**重大なこと**は、自分たちが管理しない相手(他のシステム、別の言語で書かれたクライアント、3 年後の自分たち)と共有する決定です。仕様のある標準に従い、フレームワークの便利さより優先します。HTTP のステータスと `Location`、RFC 3339 の日時、JSON と JSON Lines、終了コードと標準入出力の使い分け、PSR-4、PSR-20(`Psr\Clock\ClockInterface`)、SQL の DDL、UTF-8。`Domain/` が Carbon ではなく `DateTimeImmutable` を持ち、`Console/` が `--json` で 1 行 1 レコードを出し、API が 201 と `Location` を返すのはこの判断です。標準を破りたくなったら、標準が足りないのではなく設計が間違っている合図と考えます。

**ドメインのこと**は、正解を外から持ってこられない決定です。`Domain/` と `UseCase/` の型、値オブジェクトの不変条件、port の切り方、例外の種類、永続化する JSON のキーと DB の列。ここでは Laravel の作法も流行も参照せず、`artisan make:*` の雛形も使わず、業務の言葉で自分たちが設計します。決定はコードに残らないので ADR に書きます。

三つは重なります。永続化した JSON は、符号化(UTF-8、RFC 3339)は標準に従い、キーの意味は自分で設計します。迷ったら、その決定を 3 年後に変えるとき「Rector を回せば済むか」「相手と合意し直すか」「業務を理解し直すか」のどれになるかで振り分けてください。
...
Laravel を薄く使うとは、フレームワークに触れる層を `Http/`・`Console/`・`Persistence/`・`Provider/` に限ることであって、その層でフレームワークを避けることではありません。「何に従って決めるか」の「どうでもいいこと」を外側の層に当てはめた節です。`Domain/` と `UseCase/` がフレームワークなしで動くのは前節の通りですが、外側の層で Laravel が既に解いている問題を自前で解き直すのは、薄さではなく二重実装です。
...
7. この決定は流行・標準・ドメインのどれか。標準なら準拠する仕様の名前、ドメインなら ADR の番号は何か。
