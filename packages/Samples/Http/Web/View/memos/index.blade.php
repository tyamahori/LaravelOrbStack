...
@section('content')
  <section>
    <h1>メモ</h1>
    <p class="lead">公開したメモはS3(RustFS)にJSONとして保存し、一覧用の見出しはPostgreSQLに記録します。読み出しはRedisのキャッシュを経由し、最後に公開したメモのIDはセッションに覚えておきます。</p>
    @if (is_string($lastPublishedId))
      <p>このブラウザで最後に公開したメモ: <a href="{{ route('memos.show', ['id' => $lastPublishedId]) }}"><code>{{ $lastPublishedId }}</code></a></p>
    @else
      <p class="muted">このブラウザではまだメモを公開していません。</p>
    @endif
  </section>
